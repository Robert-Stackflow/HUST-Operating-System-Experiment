...
int sem_new(int value)
{
  for (int i = 0; i < NSEMAPHORE; i++)
  {
    ...
    {
      semaphores[i].status = SEM_USING;
      semaphores[i].count = value;
      // sprint("NNNNNNNNNNNNNN:%d TO %d\n", i, value);
      return i;
    }
  }
  return -1;
...
void sem_P(process *proc, int index)
{
  // sprint("%d PPPPPPPPPPPPPPP:%d FROM %d\n", proc->pid, index, semaphores[index].count);
  semaphores[index].count = semaphores[index].count - 1;
  if (semaphores[index].count < 0)
  {
    proc->status = BLOCKED;
    semaphores[index].sleepers |= 1 << proc->pid;
    schedule();
  }
}
void sem_V(process *proc, int index)
{
  // sprint("%d VVVVVVVVVVVVVVVV:%d FROM %d\n", proc->pid, index, semaphores[index].count);
  semaphores[index].count = semaphores[index].count + 1;
  if (semaphores[index].count <= 0)
  {
    for (int i = 0; i < NPROC; i++)
    {
      if (semaphores[index].sleepers & (1 << procs[i].pid))
      {
        semaphores[index].sleepers = semaphores[index].sleepers & (~(1 << procs[i].pid));
