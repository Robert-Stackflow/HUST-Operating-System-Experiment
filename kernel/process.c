...
int free_process(process *proc)
{
  ...
  proc->status = ZOMBIE;
  if (proc->pid!=0&&proc->parent != 0 && proc->parent->status == BLOCKED && (proc->parent->blockmap & (1 << proc->pid)))
  {
    proc->parent->status = READY;
    proc->parent->blockmap = 0;
...
int do_fork(process *parent)
{
  ...
  for (int i = 0; i < parent->total_mapped_region; i++)
  {
    ...
    case DATA_SEGMENT:
      ...
      {
        void *pa = alloc_page();
        uint64 parent_va = parent->mapped_info[i].va + k * PGSIZE;
        uint64 parent_pa = lookup_pa(parent->pagetable, parent_va);
        if (parent_pa != 0)
        {
          memcpy(pa, (void *)parent_pa, PGSIZE);
          map_pages((pagetable_t)child->pagetable, parent_va, PGSIZE, (uint64)pa, prot_to_type(PROT_WRITE | PROT_EXEC | PROT_READ, 1));
        }
      }
...
ssize_t do_wait(int pid)
{
  if (pid == -1)
  {
    ...
    for (int i = 0; i < NPROC; i++)
    {
      if (procs[i].parent != 0 && procs[i].parent->pid == current->pid)
      {
        isHaveChildren = 1;
        ...
        {
          current->blockmap |= 1 << procs[i].pid;
        }
      }
    }
    if (isHaveChildren)
